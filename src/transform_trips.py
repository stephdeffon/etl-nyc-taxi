...
if __name__ == "__main__":
    try:
        ...
        parser.add_argument(
            ...
            "--filename",
            dest="filename",
            help="filename in parquet format",
            required=True,
        )
        parser.add_argument(
            "-m", "--month", dest="month", help="month in format mm", required=True
        )
        parser.add_argument(
            "-y", "--year", dest="year", help="year in format aaaa", required=True
        )
    except argparse.ArgumentError:
        log.error("Catching an argument error")

    args = parser.parse_args()
    (filename, month, year) = (args.filename, args.month, args.year)
    transform_trips(filename, month, year)
